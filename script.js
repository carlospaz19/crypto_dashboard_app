...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function createMainTable(crypto) {
    ...
    for (const crypto in jsonPercentage) {
      if (jsonPercentage.hasOwnProperty(crypto)) {
        ...
        rowData.classList.add(
          cryptoNames.length % 2 === 0 ? "data-row-even" : "data-row-odd"
        );
      }
    }
  ...
  function createTrending(data) {
    ...
    const numberCoins = 3;
    const coins = data.coins.slice(0, numberCoins);
    let i = 0;
    ...
    coins.forEach((coin) => {
      ...
      getCoinPrice(coinId).then((data) => {
        ...
        text = `Id: ${coin.item.id}<br />Symbol: ${coin.item.symbol}<br />Rank: ${coin.item.market_cap_rank}<br />Price: ${coin.item.data.price} USD`;
  ...
  const getTrending = async () => {
    const url =
      "https://corsproxy.io/?" +
      encodeURIComponent("https://api.coingecko.com/api/v3/search/trending");
    const response = await fetch(`${url}`);
    const data = await response.json();
    return data;
  };
  ...
  const getChart = async (coinId, days) => {
    const url =
      "https://corsproxy.io/?" +
      encodeURIComponent(
